var ProjectConverter = function(SignalListener){
    ...
    this.convert = function(projectUrl) {
        ...
        if (!projectUrl && !projectJSONOffline) {
        ...
        var initializeProjectJSON = function(projectJSON) {
            ...
            try {
                ...
                _this.trackable2DObject = new AR.Trackable2DObject(_this.arTracker, "*", {
                    drawables : {
                        cam : []
                    },
    ...
    /**
     * call to update the Augmentation Loading indocator div style
     * @param finished
     */
...
var AugmentationConverter = function() {
    ...
    var generateDrawable2DOptions = function (augmentation) {
        var object = {
            offsetX: augmentation.properties.offsetX,
            offsetY: augmentation.properties.offsetY,
            opacity: augmentation.properties.opacity,
            rotation: augmentation.properties.rotation,
            scale: augmentation.properties.scale,
            zOrder: augmentation.properties.zOrder
        };

        if(augmentation.properties.rotate) {
            object.rotate = {
                heading: augmentation.properties.rotate.heading,
                roll: augmentation.properties.rotate.roll,
                tilt: augmentation.properties.rotate.tilt
            };
        }
        return object;
    };
    ...
    var convertModel = function (model, listener, signal) {
        ...
        var drawable = new AR.Model(model.uri, {
            onLoaded: onLoadedFn,
            onError: onErrorFn,
            scale: model.properties.scale,
            rotate: {
                heading: -model.properties.rotate.heading,
                roll: -model.properties.rotate.roll,
                tilt: model.properties.rotate.tilt
            },
            translate: model.properties.translate
        });
        ...
        if (image.properties.clickUrl && image.properties.clickUrl !== "") {
            ...
            var onClickFunction = function (signal, image) {
                return function() {
                    ...
                    AR.context.openInBrowser(image.properties.clickUrl);
                }
            }(signal, image);
            // in case the options already contain a click function, append the
            // new click function
