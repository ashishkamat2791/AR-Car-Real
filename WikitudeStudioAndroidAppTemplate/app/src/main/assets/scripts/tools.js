...
var createGoogleAnalyticsSignalListener = function(trackerId, projectInfo, signalTypes) {
    ...
    if (!ga) {
        throw "GOOGLE ANALYTICS - No 'ga' object accessible > missing import?";
    }
    ...
    return function (signalType, component, param) {
        ...
        if (!ga) {
            // GoogleAnalytics tracker no longer accessible
            return;
        }
...
function isSDKVersionSupported() {
    ...
    var minSDKVersion = parseVersionString("5.1.4");
...
var getBestMatchingWtcFile = function(wtcFiles) {
    ...
    var wtcVersionToUse = ["4.1", "5.0", "5.1", "5.2", "5.3"];
...
var setupReporting = function(projectInfo, signal) {
    ...
    Logger.info("ANALYTICS  - " + reportingConfigs.length + " reporting configs available");

    if (reportingConfigs && reportingConfigs.GoogleAnalytics) {
        try {
            signal.add(createGoogleAnalyticsSignalListener(reportingConfigs.GoogleAnalytics, projectInfo, signal.TYPE));
...
var createClientTracker = function(projectInfo, onTrackerLoadedFn) {
    ...
    if (!wtcFile) {
        throw "ClientTracker - required wtc NOT delivered, was searching for version but only found " + JSON.stringify(projectInfo.wtcFiles);
    } else {
        arTracker = new AR.ClientTracker(wtcFile.url, { "onLoaded": onTrackerLoadedFn, "enabled": false});
...
var createCloudTracker = function(projectInfo, onTrackerLoadedFn) {
    ...
    var arTracker = new AR.CloudTracker(projectInfo.clientToken, projectInfo.tcId, { "onLoaded": onTrackerLoadedFn });
    arTracker.isCloud = true;
    arTracker.recoCounter = 0;

    // start coudTracker
    arTracker.start = function(recoDelay) {

        recoDelay = recoDelay || 2000;

        Logger.info('Tracker - run serverside reco');
        arTracker.isTracking = true;
        arTracker.sendServerRequest = function() { if(arTracker.isTracking && !arTracker.isRecognized) arTracker.recognize(arTracker.onServerResponseArrives, arTracker.onServerErrorOccurs); };

        arTracker.onServerResponseArrives = function(recognized, response) {
            this.recoCounter++;
        ...
        arTracker.onServerErrorOccurs = function(code, errorObject) {
        ...
        setTimeout(arTracker.sendServerRequest , recoDelay);
    };

    // stop cloudTracker
    arTracker.stop = function() {
        Logger.info('Tracker - stopped (cloud)');
        clearTimeout(arTracker.sendServerRequest);
        arTracker.isTracking = false;
    };

    // restart cloudTracker
    arTracker.restart = function(recoDelay) {
        arTracker.stop();
        arTracker.start(recoDelay);
    };
